...
// @access  PUBLIC
router.get("/user_fetch/:user_id", async (req, res) => {
  const answers = await Answer.find({ user: req.params.user_id })
    .populate("user")
    .populate("question")
    .sort({ date: "desc" });
...
router.get("/question/:question_id", async (req, res) => {
  answers = await Answer.find({ question: req.params.question_id })
    .populate("user")
    .populate("question");
  res.json(answers);
  /*let final_data=[]

...
router.get("/getDislikes/:id", (req, res) => {
  let variable = {};
...
router.post(
  ...
  (req, res) => {
    ...
    variable = { answerId: req.body.answerId, userId: req.user.id };
...
router.post(
  ...
  (req, res) => {
    ...
    Likes.findOneAndDelete(variable).exec((err, result) => {
      if (err) return res.status(400).json({ success: false, err });
      res.status(200).json({ success: true });
    });
  }
...
router.post(
  ...
  (req, res) => {
    ...
    Dislikes.findOneAndDelete(variable).exec((err, result) => {
      if (err) return res.status(400).json({ success: false, err });
      res.status(200).json({ success: true });
    });
  }
